...
| 6 | Liftosaur connect screen: Settings → Liftosaur shows status, last sync, errors and workouts imported; a pasted key is checked with Liftosaur, then saved with `set_liftosaur_key` from a server action (secret key); Disconnect deletes it from Vault (`disconnect_liftosaur`) | In progress |
