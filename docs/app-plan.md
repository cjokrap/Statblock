...
| 7 | Barcode scanning: camera (ZXing, works in iPhone Safari) or a typed number. A barcode already in foods opens that food (USDA first); otherwise Open Food Facts, saved into foods (source `off`) on first log, credited on screen | In progress |
